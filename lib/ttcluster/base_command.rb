...
module TTCluster
  ...
  class BaseCommand
    ...
    def port_to_dbm_file(port)
      "#{port_to_data_dir(port)}/casket.tct"
    end
    ...
    def ttserver_command(config)
      ...
      "ttserver -dmn -host #{server_host} -port #{server_port} -sid #{server_sid} -ulog #{ulog_dir} #{ulog_limit.empty? ? '' : '-ulim '+ulog_limit} #{ulog_async ? '-uas' : ''} -mhost #{master_host} -mport #{master_port} -pid #{pid_file} -log #{log_file} -ld -rts #{rts_file} #{dbm_file}#{db_params.empty? ? '' : '#'+db_params}"
    end
